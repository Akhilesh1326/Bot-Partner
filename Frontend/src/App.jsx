...
const App = () => {
        ...
        const mockProducts = [
          {
            ...
            price: 1199.99,
            image: "https://via.placeholder.com/200x200/007bff/ffffff?text=Samsung+S24",
            rating: 4.5,
            reviews: 1250,
          ...
          },
          {
            ...
            price: 999.99,
            image: "https://via.placeholder.com/200x200/000000/ffffff?text=iPhone+15",
            rating: 4.8,
            reviews: 3400,
          ...
          },
          {
            ...
            price: 899.99,
            image: "https://via.placeholder.com/200x200/4285f4/ffffff?text=Pixel+8",
            rating: 4.3,
            reviews: 890,
          ...
          },
          {
            ...
            name: "OnePlus 12",
            price: 799.99,
            image: "https://via.placeholder.com/200x200/ff0000/ffffff?text=OnePlus+12",
            rating: 4.7,
            reviews: 2100,
            description: "Fast and affordable flagship",
          },
        ]
  ...
  const normalizeProductData = (products) => {
    return products.map((product) => ({
      ...
      image:
        product.image || `https://via.placeholder.com/200x200/007bff/ffffff?text=${encodeURIComponent(product.name)}`,
      rating: product.rating || 4,
      reviews: product.reviews || 0,
      description: product.description || product.summary || "",
      reason: "AI recommended based on your query",
    }))
  }
  ...
  const sendChatMessage = async (message) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/api/chat-LLM`, {
        message: message,
        cartItems: cartItems, // Send cart context for better responses
      })
      ...
      let botMessage = {
        id: Date.now() + 1,
        type: "bot",
        content: data.response || data.message || "I'm here to help you with your shopping!",
        isSystemMessage: false,
      }

      // Check if the response contains product data (array of products)
      if (Array.isArray(data) && data.length > 0) {
        // If the response is directly an array of products
        const normalizedProducts = normalizeProductData(data)
        botMessage = {
          ...botMessage,
          content: "Here are some products I found for you:",
          recommendations: normalizedProducts,
        }
      } else if (data.products && Array.isArray(data.products)) {
        // If products are nested in a products property
        const normalizedProducts = normalizeProductData(data.products)
        botMessage = {
          ...botMessage,
          content: data.response || data.message || "Here are some products I found for you:",
          recommendations: normalizedProducts,
        }
      } else if (data.recommendations && Array.isArray(data.recommendations)) {
        // If products are in recommendations property
        const normalizedProducts = normalizeProductData(data.recommendations)
        botMessage = {
          ...botMessage,
          content: data.response || data.message || "Here are some recommendations for you:",
          recommendations: normalizedProducts,
        }
      }
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center mb-1">
            ...
            {product.reviews > 0 && <span className="text-xs text-gray-500 ml-1">({product.reviews})</span>}
          </div>
          {reason && <p className="text-xs text-gray-500 mb-2 line-clamp-2">{reason}</p>}
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => addToCart(product)}
              className="bg-blue-600 text-white px-3 py-1 rounded-full hover:bg-blue-700 transition-colors text-xs flex items-center gap-1"
            >
        ...
        {message.recommendations && message.recommendations.length > 0 && (
          <div className="mt-3 space-y-2">
            {message.recommendations.map((rec, index) => (
              <RecommendationCard key={`${rec.id}-${index}`} product={rec} reason={rec.reason} />
            ))}
          </div>
        )}
      </div>
      {message.type === "user" && (
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
            message.isSystemMessage ? "bg-green-100" : "bg-blue-600"
          }`}
        >
          <User className={`w-4 h-4 ${message.isSystemMessage ? "text-green-600" : "text-white"}`} />
        </div>
      )}
    </div>
  )
            ...
            <div className="flex-1 overflow-y-auto p-4 min-h-0">
              {chatMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
                  ...
                  <h4 className="text-lg font-medium text-gray-800 mb-2">Welcome to your AI Shopping Assistant!</h4>
                  <p className="text-sm text-gray-600 mb-4">
                    I'm here to help you find the perfect products and answer any questions you have about your shopping
                    experience.
                  </p>
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <p className="text-sm text-blue-800">
                      💡 Try asking me about:
                      <br />• Product comparisons
                      <br />• Best deals and offers
